...
class LRUCache:
    ...
    def __init__(self, capacity):
        ...
        # Head and Tail will point to each other, making a circular list
        self.head.next = self.tail
        self.tail.prev = self.head

    def get(self, key):
        # When we get() an object, we need to move it towards the tail side of the linked list.
    ...
    def put(self, key, val):
        # If the key already exists in the cache, we remove it first,
        # then add it again to update it's tail.
        if key in self.objects:
            del self.objects[key]

        # Creating a new node and insert it at the tail (most recently used)
        node = Node(key, val)
        self.insert(node)
        self.objects[key] = node

        # Check the capacity. If over capacity,
        # remove the LRU node from the list and the hashmap
